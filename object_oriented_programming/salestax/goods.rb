...
class Goods
  ...
  def calculate_tax
    tax_percentage # Figure out our tax percentage

    unrounded = (@price * @tax_percentage) / 100.0
    @tax = (unrounded * 20).round / 20.0
    @price = ((@price + @tax) * 100).to_i / 100.0
  end

  def tax_percentage 
    @tax_percentage += 10 unless tax_exempt?
    @tax_percentage += 5 if imported?
  end

  def tax_exempt?
    exempted = ['book','chocolate','pills']
    exempted.any? { |item| @name.include? item }
  end
