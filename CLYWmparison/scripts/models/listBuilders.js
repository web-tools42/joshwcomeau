...
function build_more_info_array(object_array) {
  _.each(object_array, function(obj) {
    ...
    var more_info_template = document.getElementsByClassName(
      "yoyo_more_info_template"
    )[0];
    ...
    mi.selectAll("ul").style(
      "background-color",
      colorLuminance(obj.color_hex, -0.4)
    );

    // Add event listeners
    d3.select(new_panel)
      .on("mouseover", function() {
        var triangle = d3.select(
          "polygon.radar-chart-yoyo_" + obj.id_num
        )[0][0];
...
function build_avatar_array(object_array) {
  ...
  _.each(object_array, function(obj) {
    d3.select("#yoyo_selection_avatars")
      ...
      .attr("id", "yoyo_avatar_" + obj.id_num)
      .attr("class", "yoyo_avatar")
      .append("img")
      .attr("src", "img/" + obj.img_url);
    ...
    new_node
      .append("div")
      .attr("id", "avatar_label_" + obj.id_num)
      .attr("class", "avatar_label")
    ...
    new_node
      .insert("div", "img")
      .attr("id", "avatar_disabled_" + obj.id_num)
      .attr("class", "avatar_disabled");
...
function build_avatar_check_array(object_array) {
  _.each(avatar_list, function(avatar, index) {
    avatar
      .append("div")
      .attr("class", "avatar_checked")
      .attr("id", "avatar_checked_" + object_array[index].id_num)
